...
// Custom SMUX config for F1-F6 (415-590 nm) in parallel
// F1 (L/R) -> ADC0 (Val 1)
// F2 (L/R) -> ADC1 (Val 2)
// F3 (L/R) -> ADC2 (Val 3)
// F4 (L/R) -> ADC3 (Val 4)
...
static const uint8_t SMUX_F1_F6[20] = {
    0x30, // RAM 0x00: [6:4] F3 Left -> ADC2
    0x01, // RAM 0x01: [2:0] F1 Left -> ADC0
    0x00, // RAM 0x02:
    0x00, // RAM 0x03:
    0x06, // RAM 0x04: [2:0] F6 Left -> ADC5
    0x42, // RAM 0x05: [6:4] F4 Left -> ADC3, [2:0] F2 Left -> ADC1
    0x50, // RAM 0x06: [6:4] F5 Left -> ADC4
    0x00, // RAM 0x07:
    0x00, // RAM 0x08:
    0x50, // RAM 0x09: [6:4] F5 Right -> ADC4
    0x00, // RAM 0x0A:
    0x00, // RAM 0x0B:
    0x20, // RAM 0x0C: [6:4] F2 Right -> ADC1
    0x04, // RAM 0x0D: [2:0] F4 Right -> ADC3
    0x06, // RAM 0x0E: [2:0] F6 Right -> ADC5
    0x30, // RAM 0x0F: [6:4] F3 Right -> ADC2
    0x01, // RAM 0x10: [2:0] F1 Right -> ADC0
    0x00, // RAM 0x11:
    0x00, // RAM 0x12:
    0x00  // RAM 0x13:
};

bool Adafruit_AS7341::loadSMUXCustomTable(const uint8_t table[20]) {
...
bool Adafruit_AS7341::setupF1F4Bank(void) {
  // Public wrapper: configure SMUX for F1–F4 + Clear + NIR (low bank)
  enableSpectralMeasurement(false);
  setSMUXCommand(AS7341_SMUX_CMD_WRITE);
  setup_F1F4_Clear_NIR();
  return enableSMUX();
...
bool Adafruit_AS7341::setupF5F8Bank(void) {
  enableSpectralMeasurement(false);
  setSMUXCommand(AS7341_SMUX_CMD_WRITE);
  setup_F5F8_Clear_NIR();
  return enableSMUX();
...
bool Adafruit_AS7341::setSMUX_F1toF6() {

  // Turn off spectral measurement so SMUX config is accepted
  enableSpectralMeasurement(false);

  // Prepare SMUX write operation (sets CFG6 to 0x10)
  setSMUXCommand(AS7341_SMUX_CMD_WRITE);

  // Write the mapping into SMUX RAM
  for (uint8_t i = 0; i < 20; i++) {
    writeRegister(0x00 + i, SMUX_F1_F6[i]);
  }

  // Execute SMUX load by calling the library's function
  // This is the correct way to trigger the SMUX write.
  return enableSMUX();
}

bool Adafruit_AS7341::readF1toF6(uint16_t *buf) {

  if (!setSMUX_F1toF6()) return false;

  enableSpectralMeasurement(true);
  delayForData(0);

  // Read 6 × 16-bit values = 12 bytes
  Adafruit_BusIO_Register reg(i2c_dev, AS7341_CH0_DATA_L, 2);

  return reg.read((uint8_t*)buf, 12);
}
